...
class MainWindow(QDialog, Ui_MainWindow):
    def __init__(self):
        ...
        # self.animation.setKeyValueAt(0.3, QSize(30, 20))
...
class Ui_ScShot_Logic(QWidget, Ui_ScShot):
    ...
    def __init__(self, parent):
        ...
        try:
            ...
            self.flagEnter = False

            self.imgPrtSc = self.pixPrtSc.toImage()
            self.method_wInfo(e.globalPos().x(), e.globalPos().y()) # 这个e是事件中的，怎么在这里得到鼠标位置
            self.wInfo.show()
        except Exception as e:
            print(e)
    ...
    #     """
    #     try:
    #         if not self.flagEnter:  # 只有self.flagWTopEnter=False时，not self.flagWTopEnter 才为真
    #             self.flagEnter = True
    #             self.imgPrtSc = self.pixPrtSc.toImage()
    ...
    def paintEvent(self, event):
        # pp = QPainter(self)  # 参数一定要是拷贝的QPixmap(self.mask)
        # qc = QColor(0, 0, 0, 100)
        # pp.fillRect(self.geometry(), qc)
        ...
        #     pp.drawRect(x - 3, y - 3, 6, 6)
        #     pp.fillRect(x - 3, y - 3, 6, 6, QBrush(QColor('#00FFFF')))
    ...
    def mouseMoveEvent(self, event):
        ...
        已经在Qt设计师中设置好了
        """
        if self.flag or self.flagDrawing:
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            # self.wid.setGeometry(QRectF(self.startPoint, self.endPoint))
